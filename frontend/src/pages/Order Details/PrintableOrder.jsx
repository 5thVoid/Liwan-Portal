...
const PrintableOrder = () => {
  ...
  // Add chairman of the board (assuming role_id 9 is chairman)
  const chairmanSignature = {
    employee_id: 42, // Replace with actual chairman ID
    role_id: 9,
    timestamp: new Date().toISOString()
  };

  return (
    <div className={`${isColored ? 'bg-gradientBg' : 'bg-white'}   text-gray-700 p-8 space-y-4 print:p-2 min-h-screen`}>
       <Button 
        ...
        <div className="flex justify-between items-start">
          ...
          />
            ...
            <div className={`${isColored ? 'bg-amber-600/5' : 'bg-gray-50'} p-4 print:p-2 rounded-lg print:rounded-none`}>
                    <h1 className={`text-2xl font-bold ${isColored ? 'text-white' : 'text-gray-900'}  mb-2`}>{order.title}</h1>
                    <p className={`text-sm ${isColored ? 'text-gray-200' : 'text-gray-600'}  whitespace-pre-wrap`}>
      ...
      <div className="grid grid-cols-2 gap-6 mt-4">
        ...
        <div className={`${isColored ? 'bg-amber-600/5' : 'bg-gray-50'} p-3 rounded-lg`}>
          ...
          <div className="space-y-4">
            ...
            {/* Chairman Signature */}
            <div className={`border-t pt-3 ${isColored ? 'border-gray-200/20' :''}`}>
              <ChainSignature 
                isColored={isColored}
                employeeId={chairmanSignature.employee_id}
                signedAt={chairmanSignature.timestamp}
                roleId={chairmanSignature.role_id}
                forceRole="Chairman of the Board"
              />
            </div>
          </div>
        </div>
