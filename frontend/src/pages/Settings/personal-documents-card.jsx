...
export function PersonalDocumentsCard() {
  ...
  const fetchDocuments = useCallback(async () => {
    try {
      ...
      if (!response.data?.data?.employee?.attachment) {
        throw new Error("Invalid response format")
      }
      setDocuments(response.data.data.employee.attachment)
      const data = response.data.data.employee
      setSignature(data.signature || null) // Ensure null if no signature
    } catch (error) {
  ...
  async function handleDeleteDocument(documentType) {
    try {
      ...
      toast({
        title: t("success"),
        description: t(documentType === "signature" ? "signatureDeletedSuccessfully" : "documentDeletedSuccessfully"),
        variant: "success",
      })
